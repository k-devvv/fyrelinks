...
export default function Navbar({
  ...
  useEffect(() => {
    const saved = localStorage.getItem("fyrelinkz-theme");
  ...
  const toggleTheme = () => {
    ...
    setTheme(next);
    document.documentElement.dataset.theme = next;
    localStorage.setItem("fyrelinkz-theme", next);
  };
  useEffect(() => {
    const handle = (e: KeyboardEvent) => {
      if (e.key === "Escape") {
        if (dialog.current?.open) {
          e.preventDefault();
          dialog.current.close();
        } else if (mobileTrigger.current?.getAttribute("aria-expanded") === "true") {
          mobileTrigger.current.click();
