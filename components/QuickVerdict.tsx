...
import { Check, X, ExternalLink, Star, ShieldCheck, Flame, ArrowRight } from "lucide-react";

interface QuickVerdictProps {
  score: number; // e.g. 9.7
  badge: string; // e.g. "Editor's Choice 2025"
  summary: string;
  bottomLine: string;
  bestFor: string;
  skipIf: string;
...
export default function QuickVerdict({
  ...
  return (
    <section
      ...
      className="my-8 rounded-3xl bg-gradient-to-br from-surface-card via-surface-card to-fyre-950/25 border border-fyre-500/40 p-6 sm:p-8 shadow-2xl shadow-black/50"
    >
      {/* Top Banner */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-surface-border/80">
        <div className="flex items-center gap-3">
          <div className="flex items-center justify-center w-10 h-10 rounded-2xl bg-gradient-to-br from-fyre-500 to-rose-600 shadow-glow text-white">
            <Flame className="w-5 h-5" />
          </div>
          <div>
            <div className="flex items-center gap-2">
              <span className="text-xs font-bold uppercase tracking-wider text-fyre-400">
                FyreLinkz Lab Verdict
              </span>
